...
public class TermTest {
    ...
    private static final String[] TERMS = {"0", "+5", "-3", "+x", "-x",
            "+6x", "-9x", "+x^3", "-x^5", "+x^-2",
    ...
    private static final String[] SIMPLE_TERMS = {"3x^2", "-2x", "5", "-15x^7", "9x^0"};
    private static final String[] SIMPLE_TERMS_POWER_RULE = {"+6x", "-2", "0", "-105x^6", "0"};
    ...
    public void testPowerRule(){
        for (int i = 0; i < SIMPLE_TERMS.length; i ++){
            Assert.assertEquals(SIMPLE_TERMS_POWER_RULE[i], new Term(SIMPLE_TERMS[i]).powerRule().toString());
        }
    }
